    ...
    <Pressable style={styles.card} onPress={handleNavigate}>
      ...
      <View style={styles.imageWrapper}>
        <AutoImageSlider
          images={
            item?.gallery?.length 
              ? item.gallery.map((img) => ({ uri: img.url })) 
              : item?.gallerySummary?.length
                ? item.gallerySummary.map((img) => ({ uri: img.url }))
                : item?.heroImage 
                  ? [{ uri: item.heroImage }] 
                  : []
          }
      ...
      <View style={styles.content}>
        ...
        <View style={styles.metaRow}>
          <MetaItem
            Icon={AreaIcon}
            label="Project Area"
            value={item?.projectArea ? `${item.projectArea} Acres` : "—"}
          />
          <MetaItem
            Icon={BedIcon}
            label="Configurations"
            value={item?.bhk?.length ? `${item.bhk.join(', ')} BHK` : "—"}
          />
          <MetaItem
            Icon={ReadyToMoveIcon}
            label="Amenities"
            value={item?.amenitiesCount ? `${item.amenitiesCount}+` : "—"}
          />
        </View>
      </View>
      ...
      <View style={styles.priceBox}>
        <View>
          <Text style={styles.price}>
            {item?.priceFrom ? formatINR(item.priceFrom) : formatINR(item?.price)}
            {item?.priceTo && item.priceTo !== item.priceFrom ? ` - ${formatINR(item.priceTo)}` : ""}
          </Text>
          {item?.pricePerSqft ? (
